...
module.exports = {
conf: {name: 'unjail', aliases: ["jail-bitir", "jailbitir"], help: "!unjail @Striga/ID sebep"},
...
Penal.findOne({UserID: member.id, Active: true, Type: client.Types.Jail}, async(err, data) => { 
if(err) console.log(err)
if(!data) return message.channel.send(`Kullanıcının aktif bir Jail cezası bulunamadı.`)
await Penal.findOneAndUpdate({UserID: member.id, ID: data.ID, Type: client.Types.Jail}, { $set: {Active: false, FinishDate: Date.now(), Finisher: message.author.id}});    

message.channel.send(`${member} üyesinin hapis cezası ${message.author} tarafından "${sebep}" gerekçesiyle bitirildi.`)
...
client.channels.cache.get(logs.UnJailLog).send(embed.setDescription(`
${member} adlı üye ${message.author} tarafından "**${sebep}**" gerekçesiyle hapis cezası bitirildi. 
