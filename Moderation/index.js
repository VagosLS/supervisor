const { Discord, Collection, MessageEmbed, Client } = require('discord.js')
...
client.general = {"color1": "#389e60", "color2": "#ddd0a9", "color3": "#f23e38", "color4": "#344fa1", "color5": "#344fa1", "color6": "#51c2d5", "color7": "#6994f0", "color8": "#159ffd"}
client.generalColor = function () {return client.general[Object.keys(client.general).random()]}
Array.prototype.random = function () {return this[Math.floor((Math.random()*this.length))]};
client.warn = {"color1": "#f23e38", "color2": "#d63b3b", "color3": "#bd2121", "color4": "#df0000"}
client.warnColor = function () {return client.warn[Object.keys(client.warn).random()]}
...
Warn: "WARN",
Reklam: "REKLAM",
PermaJail: "PERMA-JAIL",
PermaMute: "PERMA-MUTE",
PermaVoiceMute: "PERMA-VOICE-MUTE",
İzinli: "İZİNLİ"
}

client.fetchBan = async (guild, userID) => {try {return await guild.fetchBan(userID);} catch (err) {return undefined;}};
...
let kufurler = ["allahoc", "allah", "Allah", "allahoç","allahamk","allahaq","0r0spuc0cu","4n4n1 sk3r1m","p1c","@n@nı skrm","evladi","orsb","orsbcogu","amnskm","anaskm","oc","abaza","abazan","ag",
...
client.NameGuard = async mesajIcerik => {
...
let SilinenMesaj = deletedMessage.guild.channels.cache.get(logs.MessageDeleteLog) 
let silinen = new MessageEmbed()
.setAuthor(deletedMessage.author.tag, deletedMessage.author.avatarURL({dynamic:true}))
.setDescription(`
\`❯\` Kullanıcı: ${deletedMessage.author} - (\`${deletedMessage.author.id}\`)
\`❯\` Kanal: **${deletedMessage.channel.name}** - (<#${deletedMessage.channel.id}>)
\`❯\` Tarih: **${moment(Date.now()).format("LLL")}**
━━━━━━━━━━━━━━━━━
\`❯\` Silinen mesaj:
\`\`\`${deletedMessage.content }\`\`\``).setColor(client.generalColor()).setFooter(settings.Footer).setTimestamp()
SilinenMesaj.send(silinen)})

client.on("messageUpdate", async(oldMessage, newMessage) => {
if (newMessage.author.bot || newMessage.channel.type === "dm") return;
let DeğişenMesaj = newMessage.guild.channels.cache.get(logs.MessageUpdateLog) 
if (oldMessage.content == newMessage.content) return;
let değişen = new MessageEmbed()
...
\`❯\` Yeni mesaj: **${newMessage.content}** `).setColor(client.generalColor()).setFooter(settings.Footer).setTimestamp()
DeğişenMesaj.send(değişen)})
